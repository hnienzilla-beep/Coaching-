...
import { Button, DecimalInput, Field, Input } from './ui'
...
export default function QuickAddFood({
  query,
  onCreated,
  title = 'Keine Treffer – neues Lebensmittel anlegen (Werte je 100 g):',
}: {
  query: string
  ...
  return (
    <div className="flex flex-col gap-1.5 p-2">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {/* Beschriftung statt Platzhalter: Die Felder starten mit 0, ein Platzhalter wäre nie zu sehen. */}
        <Field label="Protein g">
          <DecimalInput value={protein} onChange={(n) => setProtein(n ?? 0)} />
        </Field>
        <Field label="Carbs g">
          <DecimalInput value={carbs} onChange={(n) => setCarbs(n ?? 0)} />
        </Field>
        <Field label="Fett g">
          <DecimalInput value={fat} onChange={(n) => setFat(n ?? 0)} />
        </Field>
      </div>
      {existingName && (
